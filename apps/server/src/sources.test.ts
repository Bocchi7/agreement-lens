...
function textPdfBase64(text: string): string {
...
describe("source loading", () => {
  ...
  it("keeps PDF bytes available after PDF.js parsing so the raw snapshot can be written", async () => {
    const [source] = await loadSourceGraph([{
      id: `pdf-snapshot-${Date.now()}`,
      kind: "pdf",
      title: "测试用户协议",
      url: "https://example.com/agreement.pdf",
      dataBase64: textPdfBase64("User agreement terms and privacy policy."),
      selected: true,
      relation: "manual"
    }]);
    expect(source?.normalizedText).toContain("User agreement");
    expect(source?.snapshotPath).toBeTruthy();
    expect(fs.existsSync(`${snapshotDir}/${source?.snapshotPath}/raw.pdf`)).toBe(true);
  });
