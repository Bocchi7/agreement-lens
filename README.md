...
1. 用户在准备页面确认的少量协议是根来源。根来源会在分析开始时连同完整章节正文直接提供给各个 Agent，不需要 Agent 先调用工具把它们重新读一遍。
2. 解析根来源时先建立引用目录 `linkedSources`。浏览器会复用读取根来源时的动态渲染能力，将根来源直接引用的一跳页面预先读取并注册为关联来源，但不会把这些关联正文塞进根来源上下文，也不会自动开始分析它们。
3. 分析开始后，各个实际承担分析任务的 Agent 会根据当前问题检查每份材料的 `linkedSources`。需要引用其他协议、政策或规范页面时，Agent 只能复制当前材料中的精确 URL，调用统一的 `read_source({url})`；对于已由浏览器预先注册的动态页面，该工具会直接返回浏览器渲染后的完整正文。新来源返回完整正文及新的 `linkedSources` 后，Agent 可以继续按需读取相关引用。
4. 已注册来源使用 `read_source({sourceId})` 读取完整正文，或使用 `read_source({sourceId, sectionId})` 读取指定章节。读取 URL 时会复用现有的 URL 校验、重定向处理、HTML/PDF 解析、站点特化解析和快照保存逻辑；同一规范化 URL 的并发请求也会复用同一个读取任务。所有 Agent 共享当前来源注册表，因此核验器、主 Agent 和追问会话可以继续使用已读材料。
5. 来源读取失败会作为明确的工具错误返回，不会伪造正文或切换到确定性风险结论。结果页的“分析来源”会区分根来源与 Agent 按需读取的引用材料。

因此，旧的“补充材料”接口仍可作为兼容性的手动材料合并接口，但它不再是引用材料的默认分析路径；新的引用访问主路径是 Agent 工具调用。
