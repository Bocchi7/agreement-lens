...
同一条原文只能产生一个 finding：即使它同时包含“全球、永久、免费、不可撤销、商业使用、转授权、改编”等多个限定，也必须合并为一个完整告警，并在 platformAction、userImpact 和最多 2 条 evidence 中说明范围。只有触发条件、授权对象或法律后果实质不同，且能分别引用不同原文时，才可分成多个 finding。
